...
public class OrderDao {
	...
	//Lay chi tiet don hang
		...
		public List<OrderDetail>getAllChitietByMadonhangne(int imadonhang){
			String sql="select distinct ct.iMadonhang, sTensp, soluongct, sDongiact, soluongct * sDongiact from Chitietdonhang ct join Donhang dh on ct.iMadonhang = dh.iMadonhang join Sanpham sp on sp.Masp = ct.Masp where dh.iMadonhang=? group by ct.iMadonhang, sTensp, soluongct, sDongiact";
			return template.query(sql, new Object[] {imadonhang}, new RowMapper<OrderDetail>() {
		...
		public List<Order>getAllDonhang(){
			String sql="select * from donhang order by dtNgaydathang desc ";
			return template.query(sql, new Object[] {}, new RowMapper<Order>() {
				...
				public Order mapRow(ResultSet rs, int rowNum) throws SQLException {
					...
					Order ct = new Order();
					ct.setImadonhang(rs.getInt(1));
					ct.setsSDT(rs.getString(2));
					ct.setsPtthanhtoan(rs.getString(4));
					ct.setsDiachiNhanhang(rs.getString(5));
					return ct;
				}
